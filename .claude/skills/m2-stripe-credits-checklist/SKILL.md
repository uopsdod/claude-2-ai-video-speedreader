...
| # | Check | How to verify |
|---|---|---|
| A1 | `profiles` table exists with `credits_balance` column | `mcp__supabase_remote__execute_sql`: `SELECT column_name, data_type, column_default FROM information_schema.columns WHERE table_name='profiles' AND column_name='credits_balance'`. Must return one row with `data_type='numeric'` and `column_default='30'` (or similar — the default must be a positive number, ideally 30). If the query returns zero rows, either the `profiles` table itself is missing (M0/M1 didn't create it AND Step 4's CREATE-IF-NOT-EXISTS didn't run) or only the column is missing. Run the migration. |
...
| A7 | `jobs.status` CHECK constraint allows `insufficient_credits` | `mcp__supabase_remote__execute_sql`: `SELECT pg_get_constraintdef(c.oid) FROM pg_constraint c JOIN pg_class t ON c.conrelid = t.oid WHERE t.relname = 'jobs' AND c.contype = 'c'`. Output must contain `insufficient_credits`. If the table has no CHECK constraint at all, that's also acceptable (free-text column). If a CHECK constraint exists but doesn't list `insufficient_credits`, Section F's gate test will fail with a `23514 check_violation` — re-run M2 Step 4's `ALTER TABLE ... DROP CONSTRAINT ... ADD CONSTRAINT ...` block. |
...
| B8 | `STRIPE_SECRET_KEY` + `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY` are set in Vercel production env | **Vercel MCP doesn't expose env-list as of 2026-05** — verify via the Vercel **dashboard** (Project → Settings → Environment Variables → Production scope) or via `vercel env ls production` if you have the CLI. **Implicit verification:** if `/credits` rendered in B2 and Checkout Session creation works in Section E, both keys are present and correct — that's an acceptable proof in Cowork mode where the dashboard check isn't convenient. `STRIPE_WEBHOOK_SECRET` should also be present after M2 Step 9c. |
...
| C3 | One Stripe dashboard webhook endpoint exists, pointed at the production Vercel URL, subscribed to `checkout.session.completed` | **Stripe MCP doesn't expose webhook endpoint management as of 2026-05** — verify via the Stripe **dashboard** (https://dashboard.stripe.com/test/webhooks). CLI alternative: `stripe webhook_endpoints list`. Expect at least one endpoint with `enabled_events` containing `checkout.session.completed` and `url` containing your production Vercel hostname (e.g. `<repo>.vercel.app`, or the custom domain after M3). Status must be `enabled`. **Implicit verification:** if Section E's end-to-end purchase succeeds, the endpoint exists and works — that's sufficient proof. |

If C3 fails: either the endpoint doesn't exist (re-run `m2-stripe-credits` Step 9b) or it exists but its signing secret wasn't pasted into Vercel Production env. To verify the secret-to-env match without exposing the secret value: confirm Vercel Production scope has a non-empty `STRIPE_WEBHOOK_SECRET` via the Vercel dashboard. Section E will fail if either side of this is broken.

### Section D — Worker M2 patch (4 checks, all via SSM)

Same SSM-only path as M1. **Use the shorthand `commands=[...]` form** when calling through the `call_aws` MCP — its argv parser strips the JSON quoting in `--parameters '{"commands":["..."]}'`, breaking nested-quote and `&&` patterns. Keep each command simple (no `&&`, no nested quotes) and run them as separate calls. The JSON form documented in [[aws-best-practice]] Rule 2 is for direct `aws-cli` use, not the MCP wrapper.

Helper for `$INSTANCE_ID`:
```
INSTANCE_ID=$(call_aws ec2 describe-instances --filters "Name=tag:Name,Values=m1-worker" "Name=instance-state-name,Values=running" --query 'Reservations[0].Instances[0].InstanceId' --output text)
```
...
| # | Check | How to verify |
|---|---|---|
| D1 | Worker code on EC2 references duration probing | `call_aws ssm send-command --instance-ids "$INSTANCE_ID" --document-name AWS-RunShellScript --parameters commands=["grep -E 'yt-dlp.*print.*duration|probe_duration_minutes' /home/ubuntu/app/worker/worker.py"]` must return at least one match. If empty, the worker.py on EC2 hasn't been `git pull`ed since the M2 push — re-run M2 Step 10a. |
| D2 | Worker code references `credits_balance` and `insufficient_credits` | `--parameters commands=["grep -cE 'credits_balance|insufficient_credits' /home/ubuntu/app/worker/worker.py"]` must return a count ≥ 2. (Expected: at least the balance read + the status update.) |
| D3 | Worker writes `credit_transactions` with `type='deduction'` | `--parameters commands=["grep 'deduction' /home/ubuntu/app/worker/worker.py"]` must return at least one match. |
| D4 | Worker claims the job BEFORE the duration probe (Step 7a) | `--parameters commands=["grep -n 'downloading' /home/ubuntu/app/worker/worker.py"]` — confirm the `status='downloading'` update appears earlier in the file than the `probe_duration_minutes_cheap` call. If the probe runs first, a probe crash leaves the job in `pending` and creates a respawn loop. |
| D5 | Service was restarted after the pull | `--parameters commands=["systemctl show m1-distributor.service --property=ActiveEnterTimestamp"]` — the timestamp should be within the last few hours (or whenever the student finished M2 Step 10a). If it's the same as M1's restart, the student forgot to restart the service after `git pull` — restart now: `--parameters commands=["sudo systemctl restart m1-distributor.service"]`. |

If D1 fails: the EC2 has stale code. Walk the student through M2 Step 10a again.

If D4 fails: this is the critical worker-bug class from past students. The original skill probed duration *before* claiming the job — a probe crash (e.g. `float("NA")` on direct-mp4 URLs) left the job in `pending`, and the distributor's pending-poll re-spawned the same worker, which crashed again. Re-read M2 Step 7a and ensure the status flip is the first DB write.
